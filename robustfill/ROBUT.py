...
class RobState:
    ...
    def copy(self):
        return RobState(
            self.inputs, self.scratch, self.committed, self.outputs, self.past_buttons
        )
    ...
    def get_list_btns(self):
        if len(self.past_buttons) == 0:
            ...
            return np.array(
                [ALL_BUTTS_NAME_MAP[btn.name] + 1 for btn in self.past_buttons]
            )
    ...
    def get_last_btn_type(self):
        last_butt_type = (
            0
            if len(self.past_buttons) == 0
            else ALL_BUTTS_TYPES.index(self.past_buttons[-1].__class__) + 1
        )
    ...
    def to_np(self, render_kind):
        ...
        if self.past_buttons == []:
            ...
            masks = [
                self.past_buttons[-1].str_masks_to_np(str1, self)
                for str1 in self.scratch
            ]
        ...
        if render_scratch == "no":
            ...
            rendered_masks = [
                Button.str_masks_to_np_default() for _ in range(len(self.inputs))
            ]
...
class GetUpTo(Button):
    @staticmethod
    def generate_buttons():
        return [GetUpTo(name) for name in _POSSIBLE_R.keys()]
    ...
    def f(self, string):
        return string[: [m.end() for m in re.finditer(self.r[0], string)][0]]
...
class GetFrom(Button):
    @staticmethod
    def generate_buttons():
        return [GetFrom(name) for name in _POSSIBLE_R.keys()]
    ...
    def f(self, string):
        return string[[m.end() for m in re.finditer(self.r[0], string)][-1] :]
...
class GetAll(Button):
    @staticmethod
    def generate_buttons():
        return [GetAll(name) for name in _POSSIBLE_TYPES.keys()]
...
class GetSpan6(Button):
    ...
    def f(self, input_str, r1, i1, b1, r2, i2, b2):
        ...
        """
        return input_str[
            [m.end() for m in re.finditer(r1[0], input_str)][i1]
            if b1 == "End"
            else [m.start() for m in re.finditer(r1[0], input_str)][i1] : [
                m.end() for m in re.finditer(r2[0], input_str)
            ][i2]
            if b2 == "End"
            else [m.start() for m in re.finditer(r2[0], input_str)][i2]
        ]
...
class ROBENV:
    def __init__(
        ...
        inputs,
        outputs,
        render_kind={"render_scratch": "yes", "render_past_buttons": "no"},
    ):
