...
def log(x):
    return x.log() if torch and torch.is_tensor(x) else math.log(x)
...
PartialMatch = namedtuple(
    "PartialMatch",
    ["numCharacters", "score", "reported_score", "continuation", "state"],
)
...
class Pregex(namedtuple("Pregex", ["type", "arg"])):
    ...
    def match(self, string, state=None, returnPartials=False):
        ...
        while not queue.empty() and solution is None:
            ...
            for remainderMatch in current.continuation.consume(
                remainder, current.state
            ):
                ...
                if (
                    newNode not in visited_score
                    or visited_score[newNode] < newReportedScore
                ):
                    visited_score[newNode] = (
                        current.reported_score + remainderMatch.reported_score
                    )
...
class NGramLookup:
    def __init__(self, dict, max_length=None):
        if max_length is None:
            max_length = max(len(x) for x in dict.keys())
...
class CharacterClass(Pregex):
            ...
            def process(ps):
                if type(ps) is dict:
                    ...
                    if normalised:
                        ...
                        # do normalization
                        if torch and torch.is_tensor(ps):
                            return ps / ps.sum()
                        else:
                            return tuple(p / sum(ps) for p in ps)
    ...
    def sample(self, state=None):
        ...
        if return_state:
            return v, state._replace(context=state.context + v)
        else:
            return v
...
w_natural = CharacterClass(
    ascii_letters + digits, name="\\w", ps=_natural_probs(ascii_letters + digits)
)
l_natural = CharacterClass(
    ascii_lowercase, name="\\l", ps=_natural_probs(ascii_lowercase)
)
u_natural = CharacterClass(
    ascii_uppercase, name="\\u", ps=_natural_probs(ascii_uppercase)
)


class Named(Pregex):
    def __new__(cls, name, value):
        return super(Named, cls).__new__(cls, (name, value))

    def sample(self, state=None):
        return self.arg[1].sample(state)

    def consume(self, s, state):
        return self.arg[1].consume(s, state)

    def leafNodes(self):
        return self.arg[1].leafNodes()

    def flatten(self, char_map={}, escape_strings=False):
        return [self.arg[0]]

    def map(self, f):
        return Named(self.arg[0], f(self.arg[1]))
...
class String(Pregex):
    ...
    def sample(self, state=None):
        state, return_state = defaultState(state)
        if return_state:
            return self.arg, state._replace(context=state.context + self.arg)
        else:
            return self.arg
        ...
        for partialMatch in self.values[0].consume(s, state):
            if partialMatch.continuation is None:
                ...
                if len(self.values) == 1:
                    ...
                    continuation = Concat(
                        (partialMatch.continuation,) + self.values[1:]
                    )
            yield partialMatch._replace(continuation=continuation)

    def map(self, f):
        return Concat([f(v) for v in self.values])
    ...
    def flatten(self, char_map={}, escape_strings=False):
        def bracket(value):
            if (type(value) is String and len(value.arg) > 1) or type(value) == Concat:
                return (
                    [char_map.get(OPEN, OPEN)]
                    + flatten(value, char_map, escape_strings)
                    + [char_map.get(CLOSE, CLOSE)]
                )
            else:
        ...
        for i in range(len(self.values)):
            if i > 0:
                out.append(char_map.get(type(self), type(self)))
    ...
    def sample(self, state=None):
        ...
        for i in range(len(self.values)):
            values[i] = self.values[
                i
            ]  # Required so that numpy doesn't make a 2d array (because namedtuple is iterable :/ ...)
    ...
    def consume(self, s, state):
        for p, value in zip(self.ps, self.values):
            for partialMatch in value.consume(s, state):
                extraScore = log(p)
                yield partialMatch._replace(
                    score=partialMatch.score + extraScore,
                    reported_score=partialMatch.reported_score + extraScore,
                )

    def map(self, f):
        return Alt([f(v) for v in self.values], self.ps)
...
class KleeneStar(Pregex):
    ...
    def __repr__(self):
        return str(
            "(" + type(self).__name__ + " " + str(self.p) + " " + repr(self.val) + ")"
        )

    def flatten(self, char_map={}, escape_strings=False):
        if type(self.val) in (Alt, Concat) or (
            type(self.val) == String and len(self.val.arg) > 1
        ):
            return (
                [char_map.get(OPEN, OPEN)]
                + flatten(self.val, char_map, escape_strings)
                + [char_map.get(CLOSE, CLOSE), char_map.get(type(self), type(self))]
            )
        else:
            return flatten(self.val, char_map, escape_strings) + [
                char_map.get(type(self), type(self))
            ]
    ...
    def sample(self, state=None):
        ...
        while True:
            ...
            if torch and torch.is_tensor(p):
                p = p.item()
            if random.random() < p:
                break
    ...
    def map(self, f):
        return KleeneStar(f(self.val), self.p)
    ...
    def consume(self, s, state):
        for partialMatch in self.val.consume(s, state):
            if partialMatch.continuation is None:
                ...
                continuation = Concat(
                    (partialMatch.continuation, KleeneStar(self.val, self.p))
                )

            yield partialMatch._replace(continuation=continuation)

    def map(self, f):
        return Plus(f(self.val), self.p)
...
class Maybe(Pregex):
    ...
    def map(self, f):
        return Maybe(f(self.val), self.p)
        ...
        if type(seq) is str:
            if lookup is not None:
                for k, v in lookup.items():
                    if x[: len(k)] == k and type(v) is types.LambdaType:
                        return v, x[len(k) :]
            return {
                "*": KleeneStar,
                "+": Plus,
    ...
    def parseToken(seq):
        ...
        if isinstance(seq[0], Pregex):
            ...
            if seq[:2] == "\\*":
                ...
                return d_natural if natural_frequencies else d, seq[2:]
            elif seq[:2] == "\\s":
                return s_natural if natural_frequencies else s, seq[2:]
            elif seq[:2] == "\\w":
                return w_natural if natural_frequencies else w, seq[2:]
            elif seq[:2] == "\\l":
                return l_natural if natural_frequencies else l, seq[2:]
            elif seq[:2] == "\\u":
                return u_natural if natural_frequencies else u, seq[2:]
            elif seq[:1] == ".":
                return dot_natural if natural_frequencies else dot, seq[1:]
            ...
            if h == OPEN:
                ...
                if len(seq) <= 1:
                    raise ParseException()  # Lookahead
    ...
    def parse(lhs, remainder, min_precedence=0, inside_brackets=False):
        if not remainder:
            ...
            if precedence(h) < min_precedence:
                ...
                h not in (KleeneStar, Plus, Maybe, Alt)
                and type(h) is not types.LambdaType
            ):  # Atom
                ...
                while (inside_brackets and h != CLOSE) or (
                    not inside_brackets and remainder
                ):
                    if inside_brackets and not remainder:
                        raise ParseException()
                    if not inside_brackets and h == CLOSE:
                        raise ParseException()
                ...
                if type(lhs) is String and type(rhs) is String:
                    ...
                    type(lhs) is String
                    and type(rhs) is Concat
                    and type(rhs.values[0]) is String
                ):
                    return (
                        Concat((String(lhs.arg + rhs.values[0].arg),) + rhs.values[1:]),
                        remainder,
                    )
                ...
                if op in (KleeneStar, Plus, Maybe) or type(op) is types.LambdaType:
                    ...
                    while remainder and precedence(h) >= precedence(op):
                        rhs, remainder = parse(
                            rhs, remainder, precedence(op), inside_brackets
                        )
                        if remainder:
                            h, t = headtail(remainder)
