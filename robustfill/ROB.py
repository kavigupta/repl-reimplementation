...
class GetSpan:
    ...
    def str_execute(self, input_str):
        ...
        """
        return input_str[
            [m.end() for m in re.finditer(self.r1[0], input_str)][self.i1]
            if self.b1 == "End"
            else [m.start() for m in re.finditer(self.r1[0], input_str)][self.i1] : [
                m.end() for m in re.finditer(self.r2[0], input_str)
            ][self.i2]
            if self.b2 == "End"
            else [m.start() for m in re.finditer(self.r2[0], input_str)][self.i2]
        ]

    def flatten(self):
        return [
            BUTT.GetSpan1(self.r1.name),
            BUTT.GetSpan2(self.i1),
            BUTT.GetSpan3(self.b1),
            BUTT.GetSpan4(self.r2.name),
            BUTT.GetSpan5(self.i2),
            BUTT.GetSpan6(self.b2),
        ]
...
class N:
    @staticmethod
    def generate():
        choices = [GetToken, ToCase, Replace, GetUpTo, GetFrom, GetFirst, GetAll]
...
def generate_string(constraint, max_string_size=MAX_STR_LEN):
    ...
    for item in constraint_dict:
        ...
        num_to_insert = max(
            0, constraint_dict[item] - len(re.findall(reg, "".join(slist)))
        )
        if len(indices) < num_to_insert:
            return None
...
def generate_FIO(n_ios, verbose=False):
    ...
    for _ in range(20):
        ...
        try:
            ...
            out = BUTT.apply_fs(
                BUTT.RobState.new([inp], [""]), prog.flatten()
            ).committed[0]
...
def get_supervised_sample(
    n_ios=4, render_kind={"render_scratch": "yes", "render_past_buttons": "no"}
):
...
def test3():
    print(ALL_BUTTS)
    print(len(ALL_BUTTS))
    pstate = RobState.new(["(hello)123", "(mister)123"], ["HELLORE", "MISTERRE"])
    fs = [
        BUTT.GetSpan1("("),
        BUTT.GetSpan2(0),
        BUTT.GetSpan3("End"),
...
def test4():
    pstate = RobState.new(["(hello)1)23", "(mis)ter)123"], ["HELLO", "MIS"])
    fs = [
        BUTT.GetSpan1("("),
        BUTT.GetSpan2(0),
        BUTT.GetSpan3("End"),
...
def test5():
    pstate = RobState.new(["123hello123goodbye1234hola123231"], ["dontreadthis"])
    fs = [
        BUTT.GetToken1("Word"),
    ]
    pstate_new = apply_fs(pstate, fs)
...
def test11():
    ...
    for i in range(1000):
        ...
        repeat_agent.btns = (
            repeat_agent.btns[: drop_idx - 1] + repeat_agent.btns[drop_idx:]
        )
        trace = get_rollout(env, repeat_agent, 30)
...
def test14():
    ...
    S, A = get_supervised_sample(
        render_kind={"render_scratch": "yes", "render_past_buttons": "yes"}
    )
    print("scratch\n", S[-2][1])
    print("last btn type\n", S[-1][-2])
    print("past btns\n", S[-1][-1])
    ...
    S, A = get_supervised_sample(
        render_kind={"render_scratch": "no", "render_past_buttons": "yes"}
    )
    print("scratch\n", S[-2][1])
    print("last btn type\n", S[-1][-2])
    print("past btns\n", S[-1][-1])
